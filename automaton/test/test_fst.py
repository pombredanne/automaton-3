...
class MutableFSTTestCase( TestCase ):
    ...
    def test_add_state( self ):
        ...
        self.assertEqual( f.to_dict() , { 'initial':s0 , 'finals':[] , 'transitions':{ s0:{} , s1:{} } } )
        s2 = f.add_state()
    ...
    def test_add_transition( self ):
        ...
        self.assertEqual( f.to_dict() , { 'initial':s0 , 'finals':[] , 'transitions':{ s0:{ 'a':s1 } , s1:{} , s2:{} } } )
        t = f.get_target( s0 , 'a' )
        self.assertEqual( t , s1 )
        self.assertEqual( f.get_letters() , [ 'a' ] )
        # s0 b s2
        f.add_transition( s0 , 'b' , s2 )
        self.assertEqual( f.to_dict() , { 'initial':s0 , 'finals':[] , 'transitions':{ s0:{ 'a':s1 , 'b':s2 } , s1:{} , s2:{} } } )
        t = f.get_target( s0 , 'b' )
        self.assertEqual( t , s2 )
        self.assertEqual( f.get_letters() , [ 'a' , 'b' ] )
        # s1 a s0
        f.add_transition( s1 , 'a' , s0 )
        self.assertEqual( f.to_dict() , { 'initial':s0 , 'finals':[] , 'transitions':{ s0:{ 'a':s1 , 'b':s2 } , s1:{ 'a':s0 } , s2:{} } } )
        t = f.get_target( s1 , 'a' )
        self.assertEqual( t , s0 )
        # s0 a s2
        f.add_transition( s0 , 'a' , s2 )
        self.assertEqual( f.to_dict() , { 'initial':s0 , 'finals':[] , 'transitions':{ s0:{ 'a':s2 , 'b':s2 } , s1:{ 'a':s0 } , s2:{} } } )
        t = f.get_target( s0 , 'a' )
        self.assertEqual( t , s2 )
    ...
    def test_det_search_ab_babb_bb( self ):
        ...
        self.assertEqual( f.to_dict() , { 'initial':s0 , 'finals':[ s2 , s5 , s6, s7 ] , 'outputs':{ s2:[ 'ab' ] , s5:[ 'ab' ] , s6:[ 'babb' , 'bb' ] , s7:[ 'bb' ] }, 'transitions':{ s0:{ 'a':s1 , 'b':s3 } , s1:{ 'a':s1 , 'b':s2 } , s2:{ 'a':s4 , 'b':s7 } , s3:{ 'a':s4 , 'b':s7 } , s4:{ 'a':s1 , 'b':s5 } , s5:{ 'a':s4 , 'b':s6 } , s6:{ 'a':s4 , 'b':s7 } , s7:{ 'a':s4 , 'b':s7 } } } )
        self.assertEqual( f.get_letters() , [ 'a' , 'b' ] )
...
class MutableFSTDTestCase( TestCase ):
    def test_matching( self ):
        ...
        f.set_final_state( s2 ).set_final_state( s3 )
        f.add_output( s2 , 'aa' ).add_output( s3 , 'ab' )
        self.assertEqual( f.det_search( 'a ab aa' ) , [ ( 'ab' , 2 , 4 ) , ( 'aa' , 5 , 7 ) ] )
