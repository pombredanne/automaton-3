...
class BuildTrieTestCase( TestCase ):
    ...
    def test_a_b( self ):
        words = [ 'a' , 'b' ]
        t = build_trie( words )
        self.assertEqual( t.to_dict() , { 'initial':0 , 'finals':[ 1 , 2 ] , 'outputs':{ 1:[ 'a' ] , 2:[ 'b' ] } , 'transitions':{ 0:{ 'a':1 , 'b':2 } , 1:{} , 2:{} } } )
        self.assertFalse( t.accept( '' ) )
        self.assertTrue( t.accept( 'a' ) )
    ...
    def test_aa_ab( self ):
        words = [ 'aa' , 'ab' ]
        t = build_trie( words )
        self.assertEqual( t.to_dict() , { 'initial':0 , 'finals':[ 2 , 3 ] , 'outputs':{ 2:[ 'aa' ] , 3:[ 'ab' ] } , 'transitions':{ 0:{ 'a':1 } , 1:{ 'a':2 , 'b':3 } , 2:{} , 3:{} } } )
        self.assertFalse( t.accept( '' ) )
